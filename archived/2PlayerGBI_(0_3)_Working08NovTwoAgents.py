...
class QLearningAgent:
    def __init__(self, alpha=0.1, gamma=0.9
                 , epsilon=0.2, TotalNumberOfGamesToPlay=100):
...
class GridWorldGame:
    def __init__(self, BarrierPositions):
        ...
        # self.board[self.NumberOfRows-1][self.NumberOfCols-1] = 'O'   ## Always start in the bottom right corner for now No Opponent
...
#            # self.board[i][2] = '#'
    ...
    def get_moves(self, row, col):
        moves = []
        opponent = 'X' if self.current_player == 'X' else 'X'    # No Opponent - Always X's turn to play
        # opponent = 'O' if self.current_player == 'X' else 'X'  Select and line below IF you want an Opponent 
...
for r in range(1, 5):  # This will generate combinations of lengths 1 to to last number, 3 gives max combo (2, 1, 2, 2)
