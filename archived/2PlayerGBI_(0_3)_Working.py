...
class QLearningAgent:
    ...
    def choose_action(self, state, available_actions, NumberOfGamesPlayedSoFar):
        ...
        ExplorationThreshold = self.epsilon*pow(0.99, NumberOfGamesPlayedSoFar)
        ...
        while self.NumberofGamesPlayedSoFar < 100:
            ...
            if not moves:
                ...
                if self.current_player == 'X':  # if it's X's turn and there's a Q agent
                        action, MaxQValueFromAllPossibleActionsInTheCurrentState  = q_agent.choose_action(self.current_state, moves, self.NumberofGamesPlayedSoFar)   ## Select the maximum Q-value from available moves - and move state
